...
def main():
    parser = argparse.ArgumentParser(
        description='Analyze Redis Instance Key Statistics.')
    ...
    parser.add_argument('--replica_only', action='store_true',
                        help='Execute only on replica instances')
    parser.add_argument('--pretty_format', action='store_true',
                        help='Display output in a human-readable format')
    ...
    if args.cluster:
        try:
            ...
            if list(redis_info.values())[0].get('cluster_enabled') != 1:
                print(
                    "Error: Specified Redis instance is not running in cluster mode but --cluster flag was provided.")
                return
            ...
            if get_redis_cluster_keys(rc, args.batch_size, args.replica_only, args.pretty_format) == -1:
                print(
                    f"Aborted the operation on non-readonly redis at host: {args.host}")
                return

            rc.close()

            process_end_time = datetime.now()
            process_taken_time = str(
                process_end_time - process_start_time).split(".")[0]
            print(f'Process completed in {process_taken_time} (HH:MM:SS)')

        except rediscluster.exceptions.RedisClusterError as e:
            print(
                f"Error connecting to Redis cluster: {args.host}. Error message: {str(e)}")
            return
            ...
            if "ERROR sending 'cluster slots' command to redis server" in str(e):
                ...
                print(
                    f"Failed to connect to Redis cluster: {args.host}. Error message: {str(e)}")
                return
    else:
        try:
            ...
            if redis_info.get('cluster_enabled') != 0:
                print(
                    "Error: Specified Redis instance is running in cluster mode but --cluster flag was not provided.")
                return
            ...
            print(
                f"Timeout connecting to Redis: {args.host}. Error message: {str(e)}")
        except redis.exceptions.ConnectionError as e:
            print(
                f"Failed to connect to Redis: {args.host}. Error message: {str(e)}")
